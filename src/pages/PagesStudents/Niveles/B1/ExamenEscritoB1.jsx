...
function ExamenEscritoB1() {
  ...
  async function accederExamen(id) {
    try {
      ...
      if (resultado.isConfirmed) {
        navigate('/NavPageB1', { state: { examenId: id } });
      }
    } catch (error) {
  ...
  async function verificarEstadoExamen(idUsuario, idExamen) {
    try {
      const respuesta = await fetchBody('/estudiantes/obtenerEstado', 'POST', { id: idUsuario, examen: idExamen, nivel: 'B1' });
      if (respuesta.exito) {
...
export default ExamenEscritoB1