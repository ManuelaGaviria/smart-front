...
function ProgramarClaseB1() {
    ...
    useEffect(() => {
        const obtenerClases = async () => {
            try {
                const respuesta = await fetchBody('/niveles/obtenerClase', 'POST', { nivel: "B1" });
                if (respuesta.exito) {
    ...
    async function listClases() {
        try {
            ...
            if (token) {
                ...
                const respuestaClases = await fetchBody('/niveles/listarClase', 'POST', { nivel: 'B1' });
                // Obtener el estado de las clases programadas
                const respuestaEstado = await fetchBody('/estudiantes/actualizarEstadoClases', 'POST', { idEstudiante: idEst, nivel: 'B1' });
    ...
    async function listClasesProgramadas() {
        try {
            ...
            if (token) {
                ...
                // Obtener la lista de clases disponibles
                const respuesta = await fetchBody('/estudiantes/listarClaseProgramada', 'POST', { idEstudiante: idEst, nivel: 'B1' });
    ...
    async function listClasesDisponibles() {
        try {
            const respuestaClasesDisponibles = await fetchBody('/niveles/listarClase', 'POST', { nivel: "B1" });
            if (respuestaClasesDisponibles.exito) {
                ...
                if (token) {
                    ...
                    const respuestaClasesProgramadas = await fetchBody('/estudiantes/actualizarClasesDisponibles', 'POST', { id: idEst, nivel: 'B1' });
        ...
        try {
            ...
            if (respuesta.exito) {
                ...
                if (token) {
                    ...
                    const respuestaHorasAgendadas = await fetchBody('/estudiantes/obtenerHorasAgendadas', 'POST', { idEstudiante: idEst, nivel: 'B1' });
    ...
    async function programarClase() {
        ...
        if (siguienteClase > 1) {
            ...
            const examenes = await fetchBody('/niveles/obtenerExamenData', 'POST', { nivel: 'B1' });
            if (examenes.exito) {
        ...
        <motion.div
            ...
            <FullscreenCard>
                <div>
                    ...
                    <ButtonLink destino="/HorarioB1" clase="ButtonRegresar">Ver Horario</ButtonLink>
                </div>
                <br />
...
export default ProgramarClaseB1;
