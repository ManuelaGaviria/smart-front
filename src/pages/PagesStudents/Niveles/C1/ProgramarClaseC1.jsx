...
function ProgramarClaseC1() {
    ...
    useEffect(() => {
        const obtenerClases = async () => {
            try {
                const respuesta = await fetchBody('/niveles/obtenerClase', 'POST', { nivel: "C1" });
                if (respuesta.exito) {
    ...
    async function listClases() {
        try {
            ...
            if (token) {
                ...
                const respuestaClases = await fetchBody('/niveles/listarClase', 'POST', { nivel: 'C1' });
                // Obtener el estado de las clases programadas
                const respuestaEstado = await fetchBody('/estudiantes/actualizarEstadoClases', 'POST', { idEstudiante: idEst, nivel: 'C1' });
    ...
    async function listClasesDisponibles() {
        try {
            const respuestaClasesDisponibles = await fetchBody('/niveles/listarClase', 'POST', { nivel: "C1" });
            if (respuestaClasesDisponibles.exito) {
                ...
                if (token) {
                    ...
                    const respuestaClasesProgramadas = await fetchBody('/estudiantes/actualizarClasesDisponibles', 'POST', { id: idEst, nivel: 'C1' });
        ...
        try {
            ...
            if (respuesta.exito) {
                ...
                if (token) {
                    ...
                    const respuestaHorasAgendadas = await fetchBody('/estudiantes/obtenerHorasAgendadas', 'POST', { idEstudiante: idEst, nivel: 'C1' });
        ...
        <motion.div
            ...
            <FullscreenCard>
                <div>
                    ...
                    <ButtonLink destino="/HorarioC1" clase="ButtonRegresar">Ver Horario</ButtonLink>
                </div>
                <br />
...
export default ProgramarClaseC1;
