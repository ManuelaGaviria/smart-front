...
function ExamenEscritoA2() {
  ...
  async function accederExamen(id) {
    try {
      ...
      if (resultado.isConfirmed) {
        navigate('/NavPageA2', { state: { examenId: id } });
      }
    } catch (error) {
  ...
  async function verificarEstadoExamen(idUsuario, idExamen) {
    try {
      const respuesta = await fetchBody('/estudiantes/obtenerEstado', 'POST', { id: idUsuario, examen: idExamen, nivel: 'A2' });
...
export default ExamenEscritoA2