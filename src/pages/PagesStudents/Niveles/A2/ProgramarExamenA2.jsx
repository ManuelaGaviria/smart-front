...
function ProgramarExamenA2() {
    ...
    useEffect(() => {
        const obtenerExamenes = async () => {
            try {
                const respuesta = await fetchBody('/niveles/obtenerExamen', 'POST', { nivel: "A2" });
                if (respuesta.exito) {
    ...
    async function listExamenes() {
        try {
            ...
            if (token) {
                ...
                const respuestaExamenes = await fetchBody('/niveles/listarExamen', 'POST', { nivel: 'A2' });
                // Obtener el estado de los examenes programados
                const respuestaEstado = await fetchBody('/estudiantes/actualizarEstadoExamen', 'POST', { idEstudiante: idEst, nivel: 'A2' });
    ...
    async function listExamenesProgramados() {
        try {
            ...
            if (token) {
                ...
                const respuesta = await fetchBody('/estudiantes/listarExamenProgramado', 'POST', { idEstudiante: idEst, nivel: 'A2' });
    ...
    async function listExamenesDisponibles() {
        try {
            const respuestaExamenesDisponibles = await fetchBody('/niveles/listarExamen', 'POST', { nivel: "A2" });
            if (respuestaExamenesDisponibles.exito) {
                ...
                if (token) {
                    ...
                    const respuestaExamenesProgramados = await fetchBody('/estudiantes/actualizarExamenesDisponibles', 'POST', { id: idEst, nivel: 'A2' });
        ...
        try {
            ...
            if (respuesta.exito) {
                ...
                if (token) {
                    ...
                    const respuestaHorasAgendadas = await fetchBody('/estudiantes/obtenerHorasAgendadasExamen', 'POST', { idEstudiante: idEst, nivel: 'A2' });
        ...
        <motion.div
            ...
            <FullscreenCard>
                <div>
                    ...
                    <ButtonLink destino="/HorarioExamenA2" clase="ButtonRegresar">Ver Horario</ButtonLink>
                </div>
                <br></br>
...
export default ProgramarExamenA2