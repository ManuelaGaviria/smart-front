...
function VerExamenA2() {
  ...
  async function listExamenes() {
    try {
      const respuesta = await fetchBody('/niveles/listarExamen', 'POST', { nivel: "A2" });
      if (respuesta.exito) {
  ...
  async function uploadExamen(id) {
    ...
    try {
      ...
      formData.append('nivel', "A2"); // Agregar el nivel
  ...
  return (
    <motion.div
      ...
      <FullscreenCard>
            ...
            <tbody>
              {examenes.map((examen) => (
                <tr key={examen.id}>
                  ...
                  <td>{examen.clase}</td>
                  <td>
                    <ButtonLinkState clase="btn-ver" destino="/VerExamenEscritoA2" estado={{ examenId: examen.id }}>Ver</ButtonLinkState>
                  </td>
                  <td>
        ...
        <ButtonLink destino="/GestionarExamenA2" clase="ButtonRegresar">Regresar</ButtonLink>
      </FullscreenCard>
