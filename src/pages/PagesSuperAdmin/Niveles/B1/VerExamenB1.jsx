...
function VerExamenB1() {
  ...
  async function listExamenes() {
    try {
      const respuesta = await fetchBody('/niveles/listarExamen', 'POST', { nivel: "B1" });
      if (respuesta.exito) {
  ...
  async function uploadExamen(id) {
    ...
    try {
      ...
      formData.append('nivel', "B1"); // Agregar el nivel
  ...
  return (
    <motion.div
      ...
      <FullscreenCard>
            ...
            <tbody>
              {examenes.map((examen) => (
                <tr key={examen.id}>
                  ...
                  <td>{examen.clase}</td>
                  <td>
                    <ButtonLinkState clase="btn-ver" destino="/VerExamenEscritoB1" estado={{ examenId: examen.id }}>Ver</ButtonLinkState>
                  </td>
                  <td>
        ...
        <ButtonLink destino="/GestionarExamenB1" clase="ButtonRegresar">Regresar</ButtonLink>
      </FullscreenCard>
