...
function VerExamenC1() {
  ...
  async function listExamenes() {
    try {
      const respuesta = await fetchBody('/niveles/listarExamen', 'POST', { nivel: "C1" });
      if (respuesta.exito) {
  ...
  async function uploadExamen(id) {
    ...
    try {
      ...
      formData.append('nivel', "C1"); // Agregar el nivel
  ...
  return (
    <motion.div
      ...
      <FullscreenCard>
            ...
            <tbody>
              {examenes.map((examen) => (
                <tr key={examen.id}>
                  ...
                  <td>{examen.clase}</td>
                  <td>
                    <ButtonLinkState clase="btn-ver" destino="/VerExamenEscritoC1" estado={{ examenId: examen.id }}>Ver</ButtonLinkState>
                  </td>
                  <td>
        ...
        <ButtonLink destino="/GestionarExamenC1" clase="ButtonRegresar">Regresar</ButtonLink>
      </FullscreenCard>
